...
# -------------------------------------------------------------------
FROM base_install as tfrwkv_gpu_tests
...
COPY --from=make_wheel /rwkv/wheelhouse/ /rwkv/wheelhouse/
RUN pip install /rwkv/wheelhouse/*.whl

RUN if [[ -z "$SKIP_CUSTOM_OP_TESTS" ]] ; then python -c "import tensorflow_rwkv as tfrwkv; print(tfrwkv.register_all())" ; else python -c "import tensorflow_rwkv as tfrwkv; print(tfrwkv.register_all(custom_kernels=False))" ; fi

# -------------------------------------------------------------------
FROM scratch as output

COPY --from=test_wheel_in_fresh_environment /rwkv/wheelhouse/ .
