...
RUN pytest -v -n auto --durations=25 --doctest-modules ./tensorflow_rwkv \
    --cov=tensorflow_rwkv ./tensorflow_rwkv/
...
COPY --from=0 /rwkv/artifacts /artifacts

RUN pip install /artifacts/tensorflow_rwkv-*.whl
...
RUN python -c "import tensorflow_rwkv as tfrwkv; print(tfrwkv.register_all())"
